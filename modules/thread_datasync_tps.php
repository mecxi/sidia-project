...
function prepare_services_thread(&$set_thread_services, $service_local_id, $set_thread, $message){
    ...
    $notified_users = services::fetch_notify_sent($service_local_id);
    $suspended_users = services::fetch_notify_suspended($service_local_id);
    if ($users){
        $limit = 0;
        $tracker = 0;
        foreach($users as $user){
            ...
            if (thread_ctrl::has_notify_sent($user_id, $trivia_id, $notified_users)){
                ...
                if (thread_ctrl::has_notify_suspended($user_id, $suspended_users)){
